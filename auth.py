...
def create_account(key):
    if request.method == 'POST':
        ...
        query = s.query(RaphMail.temp_email).filter(RaphMail.key_email.in_([key]))
        user = User(username=str(request.form['username']), password=str(request.form['password']), email=str(query.first()[0]))
        s.add(user)
        s.commit()
        return redirect(url_for('do_admin_login'))
...
def new_account():
    if request.method == 'POST':
        ...
        Session = sessionmaker(bind=engine)
        s = Session()
        query = s.query(RaphMail.temp_email).filter(RaphMail.temp_email.in_([request.form['email']]))
        if query.first():
        ...
        mail.send(msg)
        # Stock les données
        user2 = RaphMail(key_email=str(key), temp_email=str(request.form['email']))
        s.add(user2)
        s.commit()
        return render_template('login.html')
